...
class EmailValidatorTest {
    ...
    @Test
    fun emailValidator_InvalidEmailIncorrect_MoreThanFourLettersAfterTheDot_ReturnsFalse() {
        TestCase.assertFalse(EmailValidator.isValidEmail("test@test.russ"))
    }
    ...
    fun emailValidator_EmailSame_ReturnsTrue() {
        val inputUserEmail = "user@user.com"
        val localListEmail = EmailValidator.arrayOfEmail(
            arrayOf("test@test.ru", "user@user.com")
        )
        TestCase.assertSame(inputUserEmail, localListEmail[1])
    }

    @Test
    fun emailValidator_ArrayEmailSame_ReturnsTrue() {
        val localListEmail = arrayOf("test@test.ru", "user@user.com")
        val localListEmailSame = EmailValidator.arrayOfEmail(localListEmail)
        TestCase.assertSame(localListEmail, localListEmailSame)
    }
}